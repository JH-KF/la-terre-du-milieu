...
const RoomPictureGallery = ({ className, images }) => {
  ...
  return (
    <div>
      <div
        className={`${className} block md:grid grid-cols-4 grid-rows-2 gap-4 max-h-gallery md:min-h-96`}
      >
          ...
          {images.length > 5 ? (
            <div className="z-10 absolute bottom-4 right-2">
              <BaseSecondaryButton
                ...
                onClick={() => setActiveModalImageIndex(5)}
              />
            </div>
          ) : null}
        </div>
      ...
      <ModalGallery
        images={images}
        index={activeModalImageIndex}
        setIndex={setActiveModalImageIndex}
      />
    </div>
  )
}
