...
const Room = ({ pageContext, data }) => {
  ...
  pageContext.room.informations.forEach(i => {
    informations.push(
      <BaseDescription description={i} key={i} className="mb-4" />
    )
  })
  const otherRooms = []
  pageContext.otherRooms.forEach(room => {
    otherRooms.push(
      <BaseRoomCard
        ...
        thumbnailImage={
          data.roomThumbnailImages.nodes
  ...
  return (
    <>
      <Seo
        title={pageContext.room.title}
        lang={i18n.language}
        description={pageContext.room.description}
        path={pageContext.roomPath}
        image={getSrc(data.roomImages.nodes[0])}
      />
          ...
          <div className="max-w-screen-xl mx-auto px-6 md:px-12">
            ...
            <div>
              ...
              <div className="block md:grid gap-x-6 md:grid-rows-1 md:grid-cols-3">
                ...
                <BaseParchment
                  light={true}
                  className="p-4 md:p-6 block md:col-start-1 md:col-end-3 md:row-start-1 md:row-end-1"
                >
                  {!pageContext.isAvailable ? (
                    <BaseDescription
                  ...
                  <BaseDescription
                    ...
                    description={`"${pageContext.room.quote}"`}
                  />
                  <BaseDescription
                    className="font-calligraphy font-semibold"
                    description={pageContext.room.quoteAuthor}
                  />
                </BaseParchment>
              </div>
            </div>
...
export const query = graphql`
  query ($imagesPath: String!) {
    roomImages: allFile(filter: { relativeDirectory: { eq: $imagesPath } }) {
      nodes {
        name
        childImageSharp {
